...
if [ "$*" == "" ]
    then
        ...
        echo
        echo "Example: $ bash install_nexon.bash 2wd a1"
        echo "Example: $ bash install_nexon.bash 2wd a1 realsense"
        echo "Example: $ bash install_nexon.bash 2wd - realsense"
        echo "Example: $ bash install_nexon.bash 2wd"
...
if [[ "$BASE" != "ci" && "$LASER_SENSOR" != "" && "$LASER_SENSOR" != "-" ]] && !(printf '%s\n' "${LASER_SENSOR_ARRAY[@]}" | grep -xq $LASER_SENSOR)
    then
        echo "Invalid nexon laser sensor: $LASER_SENSOR"
        echo
        echo "Valid Options:"
...
if [[ "$BASE" != "ci" && "$DEPTH_SENSOR" != "" ]] && !(printf '%s\n' "${DEPTH_SENSOR_ARRAY[@]}" | grep -xq $DEPTH_SENSOR)
    then
        echo "Invalid nexon depth sensor: $DEPTH_SENSOR"
        echo
        echo "Valid Options:"
...
if [[ "$BASE" != "ci" ]]
    then
        echo
        echo "You are installing nexon on your robot computer."
        echo
        echo "===========SUMMARY============"
        ...
        echo "Create a nexon_ws on your $HOME directory."
        echo "Install nexon ROS2 dependencies."
...
cd $WORKSPACE/src/nexon/nexon_gazebo
touch COLCON_IGNORE
...
if [[ "$BASE" != "ci" ]]
    then
        ### 1. Robot Type
        echo "export nexon_BASE=$BASE" >> ~/.bashrc
        ### 2. Sensors
        if [[ "$LASER_SENSOR" != "-" ||  "$LASER_SENSOR" != "" ]]
            then
                echo "export nexon_LASER_SENSOR=$LASER_SENSOR" >> ~/.bashrc
        fi

        if [[ "$DEPTH_SENSOR" != "-" ||  "$DEPTH_SENSOR" != "" ]]
            then
            echo "export nexon_DEPTH_SENSOR=$DEPTH_SENSOR" >> ~/.bashrc
        fi
        echo
        echo "Do you want to add sourcing of nexon_ws on your ~/.bashrc?"
        echo -n "Yes [y] or No [n]: " 
        read reply
        if [[ "$reply" == "y" || "$reply" == "Y" ]]
            then
                echo "source \$HOME/nexon_ws/install/setup.bash" >> ~/.bashrc
        else
            echo
            echo "Remember to run $ source ~/nexon_ws/install/setup.bash every time you open a terminal."
        fi
fi
