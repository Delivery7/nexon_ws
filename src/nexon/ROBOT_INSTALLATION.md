## Manual installation of nexon package on robot computer
...
### 2. Download nexon and its dependencies:
...
If you're installing this on the robot's computer or you don't need to run Gazebo at all, you can skip nexon_gazebo package by creating a COLCON_IGNORE file:

    cd src/nexon/nexon_gazebo
    touch COLCON_IGNORE

#### 2.3 Install nexon package:
...
* microxrcedds_agent dependency checks are skipped to prevent this [issue](https://github.com/micro-ROS/micro_ros_setup/issues/138) of finding its keys. This means that you have to always add `--skip-keys microxrcedds_agent` whenever you have to run `rosdep install` on the ROS2 workspace where you installed nexon.

## ENV Variables
### 1. Robot Type
Set nexon_BASE env variable to the type of robot base that you want to use. This is not required if you're using a custom URDF. Available env variables are *2wd*, *4wd*, and *mecanum*. For example:

    echo "export nexon_BASE=2wd" >> ~/.bashrc

### 2. Sensors
#### 2.1 Depth Sensor (Optional)
The Nav2 config file has been configured to support [Voxel Layer](https://navigation.ros.org/configuration/packages/costmap-plugins/voxel.html) for marking 3D obstacles in the Local Costmap using a depth sensor. To enable one of the tested depth sensor's launch files in bringup.launch.py, export the depth sensor you're using to `nexon_DEPTH_SENSOR` env variable.
...
For example:
    ...
    echo "export nexon_DEPTH_SENSOR=realsense" >> ~/.bashrc

#### 2.2 Laser Sensor (Optional)
The launch files of the tested laser sensors have already been added in bringup.launch.py. You can enable one of these sensors by exporting the laser sensor you're using to `nexon_LASER_SENSOR` env variable.
...
For example:
    ...
    echo "export nexon_LASER_SENSOR=rplidar" >> ~/.bashrc

If you export a depth sensor to `nexon_LASER_SENSOR`, the launch file will run [depthimage_to_laserscan](https://github.com/ros-perception/depthimage_to_laserscan) to convert the depth sensor's depth image to laser.
...
and paste the following:
    ...
    export nexon_BASE=<your_robot_type>
    export nexon_LASER_SENSOR=<your_supported_sensor> #(optional)
...
and paste the following:
    ...
    Type=simple
    User=<user>
    ExecStart=/bin/sh -c ". /opt/ros/<your_ros_distro>/setup.sh;. /etc/ros/env.sh;. /home/<user>/<your_ws>/install/setup.sh; ros2 launch nexon_bringup bringup.launch.py joy:=true"
...
- `your_ws` with the location of the ros2 ws where you installed nexon
