# nexon
![nexon](docs/nexon.gif)

nexon is a ROS2 implementation of the [linorobot](https://github.com/linorobot/linorobot) package for building custom robots with 2WD, 4WD, or Mecanum drive configurations. This package provides launch files for Nav2 integration and includes a complete simulation pipeline in Gazebo.
...
![nexon_architecture](docs/nexon_launchfiles.png)

Assuming you're using one of the tested sensors, nexon automatically launches the necessary hardware drivers, with the topics being conveniently matched with the topics available in Gazebo. This allows users to define parameters for high level applications (ie. Nav2 SlamToolbox, AMCL) that are common to both virtual and physical robots.

The image below summarizes the topics available after running **bringup.launch.py**.
![nexon_microcontroller](docs/microcontroller_architecture.png)

An in-depth tutorial on how to build the robot is available in [nexon_hardware](https://github.com/linorobot/nexon_hardware).
...
### 1. Robot Computer - nexon Package
The easiest way to install this package on the robot computer is to run the bash script found in this package's root directory. It will install all the dependencies, set the ENV variables for the robot base and sensors, and create a nexon_ws (robot_computer_ws) on the robot computer's `$HOME` directory. If you're using a ZED camera with a Jetson Nano, you must create a custom Ubuntu 20.04 image for CUDA and the GPU driver to work. Here's a quick [guide](./ROBOT_INSTALLATION.md#1-creating-jetson-nano-image) on how to create a custom image for Jetson Nano.

    source /opt/ros/<ros_distro>/setup.bash
    cd /tmp
    wget https://raw.githubusercontent.com/linorobot/nexon/${ROS_DISTRO}/install_nexon.bash
    bash install_nexon.bash <robot_type> <laser_sensor> <depth_sensor>
    source ~/.bashrc
...
#### 2.1 Install nexon Package
Install nexon package on the host machine:

    cd <host_machine_ws>
    git clone -b $ROS_DISTRO https://github.com/linorobot/nexon src/nexon
    rosdep update && rosdep install --from-path src --ignore-src -y --skip-keys microxrcedds_agent --skip-keys micro_ros_agent
    colcon build
    source install/setup.bash

* microxrcedds_agent and micro_ros_agent dependency checks are skipped to prevent this [issue](https://github.com/micro-ROS/micro_ros_setup/issues/138) of finding its keys. This means that you have to always add `--skip-keys microxrcedds_agent --skip-keys micro_ros_agent` whenever you have to run `rosdep install` on the ROS2 workspace where you installed nexon. 

#### 2.2 Define Robot Type
Set nexon_BASE env variable to the type of robot base used. Available env variables are *2wd*, *4wd*, and *mecanum*. For example:

    echo "export nexon_BASE=2wd" >> ~/.bashrc
    source ~/.bashrc
...
Install [nexon_viz](https://github.com/linorobot/nexon_viz) package to visualize the robot remotely specifically when creating a map or initializing/sending goal poses to the robot. The package has been separated to minimize the installation required if you're not using the simulation tools on the host machine.

    cd <host_machine_ws>
    git clone https://github.com/linorobot/nexon_viz src/nexon_viz
    rosdep update && rosdep install --from-path src --ignore-src -y 
    colcon build
    source install/setup.bash

## Hardware and Robot Firmware
All the hardware documentation and robot microcontroller's firmware can be found [here](https://github.com/linorobot/nexon_hardware).

## URDF
### 1. Define robot properties
[nexon_description](./nexon_description) package has parameterized xacro files that can help you kickstart writing the robot's URDF. Open <robot_type>.properties.urdf.xacro in [nexon_description/urdf](./nexon_description/urdf) directory and change the values according to the robot's specification/dimensions. All pose definitions must be measured from the `base_link` (center of base) and wheel positions (ie `wheel_pos_x`) are referring to wheel 1.

For custom URDFs, you can change the `urdf_path` in [description.launch.py](./nexon_description/launch/description.launch.py) found in nexon_description/launch directory. 
...
#### 2.1 Publish the URDF from the robot computer:
    ...
    ros2 launch nexon_description description.launch.py
...
- **rviz** - Set to true to visualize the robot in rviz2 and only if you're configuring the URDF from the host machine. For example:
        ...
        ros2 launch nexon_description description.launch.py rviz:=true
...
The `rviz` argument on description.launch.py won't work on headless setup but you can visualize the robot remotely from the host machine:
    ...
    ros2 launch nexon_viz robot_model.launch.py
...
- **base_serial_port** - Serial port of the robot's microcontroller. The assumed value is `/dev/ttyACM0`. Otherwise, change the default value to the correct serial port. For example:
    ...
    ```
    ros2 launch nexon_bringup bringup.launch.py base_serial_port:=/dev/ttyACM1
    ```

- **micro_ros_baudrate** - micro-ROS serial baudrate. default 115200.

    ```
    ros2 launch nexon_bringup bringup.launch.py base_serial_port:=/dev/ttyUSB0 micro_ros_baudrate:=921600
    ```
...
- **micro_ros_port** - micro-ROS udp/tcp port number. default 8888.
    ...
    ros2 launch nexon_bringup bringup.launch.py micro_ros_transport:=udp4 micro_ros_port:=8888
    ```
...
- **madgwick** - Set to true to enable magnetometer support. The madgwick filter will fuse imu/data_raw and imu/mag to imu/data. You may visualize the IMU and manetometer by [enable the IMU and magetometer plug-ins](https://automaticaddison.com/how-to-publish-imu-data-using-ros-and-the-bno055-imu-sensor/) in RVIZ2. The ekf filter configuration will need update, as only 'vyaw' is enabled in the default configuration. Both IMU and magnetometer must be calibrated, otherwise the robot's pose will rotate.
    ...
    ros2 launch nexon_bringup bringup.launch.py madgwick:=true orientation_stddev:=0.01

    nexon_ws/src/nexon/nexon_base/config/ekf.yaml
        imu0: imu/data
...
#### 1.1b Using Gazebo:
    ...
    ros2 launch nexon_gazebo gazebo.launch.py

nexon_bringup.launch.py or gazebo.launch.py must always be run on a separate terminal before creating a map or robot navigation when working on a real robot or gazebo simulation respectively.
...
Pass `joy` argument to the launch file and set it to true to enable the joystick. For example:
    ...
    ros2 launch nexon_bringup bringup.launch.py joy:=true
...
For example:
    ...
    ros2 launch nexon_navigation slam.launch.py rviz:=true sim:=true
...
The `rviz` argument on slam.launch.py won't work on headless setup but you can visualize the robot remotely from the host machine:
    ...
    ros2 launch nexon_viz slam.launch.py
...
#### 3.3 Save the map
    ...
    cd nexon/nexon_navigation/maps
...
Open nexon/nexon_navigation/launch/navigation.launch.py and change *MAP_NAME* to the name of the newly created map. Build the robot computer's workspace once done:
...
Alternatively, `map` argument can be used when launching Nav2 (next step) to dynamically load map files. For example:
    ...
    ros2 launch nexon_navigation navigation.launch.py map:=<path_to_map_file>/<map_name>.yaml
...
The `rviz` argument for navigation.launch.py won't work on headless setup but you can visualize the robot remotely from the host machine:
    ...
    ros2 launch nexon_viz navigation.launch.py
...
- Try to up `transform_timeout` by 0.1 in nexon_navigation/config/slam.yaml until the warning is gone.
