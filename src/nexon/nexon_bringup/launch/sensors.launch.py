...
def generate_launch_description():
    laser_sensor_name = os.getenv('NEXON_LASER_SENSOR', 'lasers.launch.py')
    depth_sensor_name = os.getenv('NEXON_DEPTH_SENSOR', '')
    
    fake_laser_config_path = PathJoinSubstitution(
        [FindPackageShare('nexon_bringup'), 'config', 'fake_laser.yaml']
    )
    ...
    depth_launch_path = PathJoinSubstitution(
        [FindPackageShare('nexon_bringup'), 'launch', 'depth.launch.py']
    )
